...
public static class UniGif
{
	...
	private struct GifData
	{
		...
		public string signature => new string(new char[3]
		{
			(char)m_sig0,
			(char)m_sig1,
			(char)m_sig2
		...
		public string version => new string(new char[3]
		{
			(char)m_ver0,
			(char)m_ver1,
			(char)m_ver2
	...
	private struct ApplicationExtension
	{
		...
		public string applicationIdentifier => new string(new char[8]
		{
			(char)m_appId1,
			(char)m_appId2,
			(char)m_appId3,
		...
		public string applicationAuthenticationCode => new string(new char[3]
		{
			(char)m_appAuthCode1,
			(char)m_appAuthCode2,
			(char)m_appAuthCode3
	...
	public static IEnumerator GetDefaultTextureListCoroutine(byte[] bytes, Action<List<GifTexture>, int, int, int> callback, UniGifImage uniGifImage, FilterMode filterMode = FilterMode.Bilinear, TextureWrapMode wrapMode = TextureWrapMode.Clamp, bool debugLog = false)
	{
		int loopCount = -1;
		int width = 0;
		int height = 0;
		...
		Debug.Log("[GetTextureListCoroutine] SetGifData Done!");
		List<GifTexture> gifTexList = null;
		yield return DecodeDefaultTextureCoroutine(gifData, delegate(List<GifTexture> result)
		{
			gifTexList = result;
		}, filterMode, wrapMode, uniGifImage);
		...
		Debug.Log("[GetTextureListCoroutine] done!");
	}

	public static IEnumerator GetTextureListCoroutine(byte[] bytes, Action<List<GifTexture>, int, int, int> callback, UniGifImage uniGifImage, FilterMode filterMode = FilterMode.Bilinear, TextureWrapMode wrapMode = TextureWrapMode.Clamp, bool debugLog = false)
	{
		int loopCount = -1;
		int width = 0;
		int height = 0;
		...
		Debug.Log("[GetTextureListCoroutine] SetGifData Done!");
		uniGifImage.frames = new List<LoadGifImage.GifFrame>(gifData.m_imageBlockList.Count);
		List<GifTexture> gifTexList = null;
	...
	private static IEnumerator DecodeDefaultTextureCoroutine(GifData gifData, Action<List<GifTexture>> callback, FilterMode filterMode, TextureWrapMode wrapMode, UniGifImage gifImage)
	{
		if (gifData.m_imageBlockList != null && gifData.m_imageBlockList.Count >= 1)
		{
			Debug.Log("[DecodeDefaultTextureCoroutine] Start!");
			...
			int imgIndex = 0;
			byte[] decodedData = GetDecodedData(gifData.m_imageBlockList[imgIndex]);
			...
			Color32 bgColor;
			List<byte[]> colorTable = GetColorTableAndSetBgColor(gifData, gifData.m_imageBlockList[imgIndex], transparentIndex, out bgColor);
			yield return 0;
			bool filledTexture;
			...
			{
				SetTexturePixelRow(tex, num, gifData.m_imageBlockList[imgIndex], decodedData, ref dataIndex, colorTable, bgColor, transparentIndex, filledTexture);
			}
			tex.Apply();
			yield return 0;
			float delaySec = GetDelaySec(graphicCtrlEx);
			gifTexList.Add(new GifTexture(tex, delaySec));
			gifImage.CopyFrame(gifTexList[imgIndex].m_texture2d, gifData.m_logicalScreenWidth, gifData.m_logicalScreenHeight);
			UIManager.Instance.SetLoadingCircle(isLoading: false);
			callback?.Invoke(gifTexList);
			Resources.UnloadUnusedAssets();
		}
	}

	private static IEnumerator DecodeTextureCoroutine(GifData gifData, Action<List<GifTexture>> callback, FilterMode filterMode, TextureWrapMode wrapMode, UniGifImage gifImage)
	{
		if (gifData.m_imageBlockList == null || gifData.m_imageBlockList.Count < 1)
		{
			yield break;
		...
		List<ushort> disposalMethodList = new List<ushort>(gifData.m_imageBlockList.Count);
		int imgIndex = 0;
		Debug.Log("[DecodeTextureCoroutine] m_imageBlockList Count: " + gifData.m_imageBlockList.Count);
		for (int i = 0; i < gifData.m_imageBlockList.Count; i++)
		{
			...
			gifTexList.Add(new GifTexture(tex, delaySec));
			yield return 0;
			if (UIManager.Instance.currentScene == UIManager.SceneState.MediaPopup)
			{
				gifImage.CopyFrame(gifTexList[i].m_texture2d, gifData.m_logicalScreenWidth, gifData.m_logicalScreenHeight);
				gifImage.SetDelayTime(gifData.m_graphicCtrlExList[i].m_delayTime);
				yield return 0;
			}
			imgIndex++;
			Resources.UnloadUnusedAssets();
	...
	{
		filledTexture = false;
		Texture2D texture2D = new Texture2D(gifData.m_logicalScreenWidth, gifData.m_logicalScreenHeight, TextureFormat.ARGB32, mipChain: false);
	...
	private static void SetImageBlock(byte[] gifBytes, ref int byteIndex, ref GifData gifData)
	{
		...
		{
			m_imageSeparator = gifBytes[byteIndex]
		};
		byteIndex++;
	...
	{
		GraphicControlExtension item = new GraphicControlExtension
		{
			m_extensionIntroducer = gifBytes[byteIndex]
		};
		byteIndex++;
	...
	{
		CommentExtension item = new CommentExtension
		{
			m_extensionIntroducer = gifBytes[byteIndex]
		};
		byteIndex++;
	...
	{
		PlainTextExtension item = new PlainTextExtension
		{
			m_extensionIntroducer = gifBytes[byteIndex]
		};
		byteIndex++;
